    ...
    <div class="container">
      ...
      <form id="bookingForm">
        <div class="grid">
          ...
          <!-- Changed from plain text field to drop down
          Legacy name:
          id="fullName" is retained for compatibility.
          The selected value is now a tutor ID, not a person's name.  -->
          <div class="field">
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateTutorDropdown() {
      google.script.run
        .withSuccessHandler(function (tutors) {
          ...
          // note: 'fullname' is actually 'tutorID'
          const tutorSelect = document.getElementById('fullName');
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (formData.recurring === 'Yes') {
        if (!formData.frequency || !formData.repeatUntil) {
          showMessage(
            'Please complete the recurring booking fields.',
            'error'
          );
